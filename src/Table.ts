...
type TableMeta <T> = {
  asBuilder?: <T>(col: ColumnMeta<T>, quoted: boolean) => string
  alias?: string
  columns: T
  name: string
  quote?: string
...
const defaultAsBuilder = <T>(col: ColumnMeta<T>, quoted: boolean) => {
  let q = quoted ? (col.table.quote || '') : ''
...
const as = <T>(col: ColumnMeta<T>): string => col.toString() + ' as ' + (col.table.asBuilder || defaultAsBuilder)(col, true)
...
const getAs = <T>(col: ColumnMeta<T>): string => (col.table.asBuilder || defaultAsBuilder)(col, false)
